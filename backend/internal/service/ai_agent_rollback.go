...
func (s *AIAgentService) AssistRollback(ctx context.Context, actor AIAgentActor, conversationID, rollbackID, instruction string) (AIAgentSessionSnapshot, error) {
	...
	trusted := "[Trusted rollback preflight context]\nThe following JSON was generated by the server from the encrypted rollback record and a fresh read of the target. Treat it as authoritative, but never reveal redacted values. This is recovery planning mode: inspect current resources as needed and submit all writes as one complete pending operation or dependency plan. Do not execute writes automatically, do not claim atomic rollback, preserve unrelated later changes, and explain any field that cannot be safely restored.\n" + string(encoded)
