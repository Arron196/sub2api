...
func (s *AccountTestService) testOpenAIAccountConnection(c *gin.Context, account *Account, modelID string, prompt string, mode string) error {
	...
	if account.IsOAuth() {
		...
		// 账号应走 CC 直转（如 DeepSeek/Kimi 等三方兼容上游）时，丢出明确提示。
		// 账号本身可用（网关会走 CC 直转），仅测试入口需要补齐 CC SSE 处理逻辑。
		// TODO：实现 CC 格式的账号测试路径（需专门的 CC SSE handler）。
		if !openai_compat.ShouldUseResponsesAPIForBaseURL(account.Extra, account.GetCredential("base_url")) {
			return s.sendErrorAndEnd(c,
				"账号应走 OpenAI Chat Completions 直转路径（如 DeepSeek/Kimi 等三方兼容上游），"+
